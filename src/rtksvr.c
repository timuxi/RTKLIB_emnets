 ...
 * @param obs
 */
extern void mSaveSolution(rtksvr_t *svr, obs_t obs)
{

    int k, kj;
    int nu, nr;
    char c[32];
    FILE *fp = fopen("/home/pi/xcy/RTKLIB/app/consapp/rtkrcv/test.txt", "a+");

    mWrite(intReverChar(c, svr->rtk.sol.time.time), fp);
    /*输出浮点解*/
    for (kj = 0; kj < 3; kj++)
    {
        mWrite(doubleReverChar(c, svr->rtk.x[kj]), fp);
    }
    /*输出固定解*/
    for (kj = 0; kj < 3; kj++)
    {
        mWrite(doubleReverChar(c, svr->rtk.xa[kj]), fp);
    }
    /*输出最终解*/
    for (kj = 0; kj < 3; kj++)
    {
    ...
    /*解的状态*/
    mWrite(intReverChar(c, svr->rtk.sol.stat), fp);
    /*参与解算的共视卫星数*/
    mWrite(intReverChar(c, svr->rtk.sol.ns), fp);
    /*解算出来的整周模糊度*/
    mWrite(intReverChar(c, svr->rtk.sol.nb), fp);
    fputs("\n", fp);

    for (k = 0; k < obs.n; k++)
    {
        int satNumber = obs.data[k].sat - 1;
        /*只解算有效卫星*/
        if (svr->rtk.ssat[satNumber].vs != 1)
        {
            return;
        }
        /*卫星编号*/
        mWrite(intReverChar(c, satNumber), fp);
        mWrite(intReverChar(c, obs.data[k].rcv), fp);
        /*方向角和仰角*/
        mWrite(doubleReverChar(c, svr->rtk.ssat[satNumber].azel[0]), fp);
        mWrite(doubleReverChar(c, svr->rtk.ssat[satNumber].azel[1]), fp);
        /*卫星snr*/
        for (kj = 0; kj < NFREQ; kj++)
        {
            mWrite(intReverChar(c, obs.data[k].SNR[kj]), fp);
        }
        fputs("\n", fp);
    }
...
extern char *doubleReverChar(char *c, double dou)
{
    if (dou == 0.0)
    {
        return "0";
...
extern char *intReverChar(char *c, int in)
{
    sprintf(c, "%d", in);
    return c;
}

extern char mWrite(char *data, FILE *fp)
{
    fputs(data, fp);
    fputs(",", fp);
}
